...
def main():
    ...
    ia: Union[
        IMDbHTTPAccessSystem, IMDbS3AccessSystem, IMDbSqlAccessSystem
    ] = imdb.IMDb("https")
    ...
    for show_cfg in config["series"]:
        ...
        try:
            ...
            print(
                Fore.RED
                + f"There was an error parsing a date value for the show {series_name}"
            )
            print(e)
            continue
        ...
        if next_season_earliest_release < datetime.now():
            print(
                Fore.BLUE
                + f"Season {next_unwatched_season} of {series_name} is out already!"
            )
